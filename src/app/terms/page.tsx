...
export default function TermsPage() {
  const { terms: t } = useExt();
  return (
    <main className="min-h-screen bg-brew-black text-brew-ivory px-6 py-20 md:px-12 lg:px-24">
      <div className="max-w-2xl mx-auto space-y-10">
        <header className="space-y-3">
          <p className="text-[10px] uppercase tracking-[0.4em] text-brew-warm-gray">{t.eyebrow}</p>
          <h1 className="text-3xl md:text-4xl font-medium">{t.headline}</h1>
          <p className="text-xs text-brew-warm-gray">{t.updated}</p>
        </header>
